...
| `round`       | Number         | reviewer          | Review iteration counter (spec + code reviews). Drives `max_rounds`; reset to 0 when the spec is approved and on a human bug bounce, so each loop gets a full budget. |
