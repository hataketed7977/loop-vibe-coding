...
| `status`      | Single select  | coder / reviewer  | Lifecycle stage — see options below. Agents may never set `done`.       |
...
| `resolution`  | Text (multi)   | coder             | Per-item judgement of feedback + what was changed + checks run.         |
| `round`       | Number         | reviewer          | Review iteration counter. Drives the `max_rounds` circuit breaker.      |
| `test_report` | Text (multi)   | coder / human     | Machine test/lint output + the human's acceptance notes.                |
...
| `implementing` | Spec approved, being implemented                     | coder           |
...
| `done`         | Accepted, archived & committed (human only)          | —               |
