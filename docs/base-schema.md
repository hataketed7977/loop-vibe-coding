...
| `round`       | Number         | reviewer (increment + reset) · human (reset) · coder (reset only) | Review iteration counter (spec + code reviews). Drives `max_rounds`. Each writer has ONE specific job: the **reviewer** increments it on every review and resets it to 0 when it approves a spec (`spec → implementing`); a **human** resets it to 0 when bouncing a bug back (`testing → implementing`); the **coder** never increments — it only force-resets to 0 to self-heal a human bug bounce that arrived with a stale non-zero value. No role writes `round` outside these cases. |
